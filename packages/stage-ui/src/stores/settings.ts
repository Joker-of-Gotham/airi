...
export const useSettings = defineStore('settings', () => {
  ...
  const language = useLocalStorageManualReset('settings/language', '')

  const stageModelSelected = useLocalStorageManualReset<string | undefined>('settings/stage/model', 'preset-live2d-1')
  ...
  const stageViewControlsEnabled = refManualReset(false)

  const live2dDisableFocus = useLocalStorageManualReset('settings/live2d/disable-focus', false)
  const live2dIdleAnimationEnabled = useLocalStorageManualReset('settings/live2d/idle-animation-enabled', true)
  const live2dAutoBlinkEnabled = useLocalStorageManualReset('settings/live2d/auto-blink-enabled', true)
  const live2dForceAutoBlinkEnabled = useLocalStorageManualReset('settings/live2d/force-auto-blink-enabled', false)
  const live2dShadowEnabled = useLocalStorageManualReset('settings/live2d/shadow-enabled', true)

  const disableTransitions = useLocalStorageManualReset('settings/disable-transitions', true)
  const usePageSpecificTransitions = useLocalStorageManualReset('settings/use-page-specific-transitions', true)

  const themeColorsHue = useLocalStorageManualReset('settings/theme/colors/hue', DEFAULT_THEME_COLORS_HUE)
  const themeColorsHueDynamic = useLocalStorageManualReset('settings/theme/colors/hue-dynamic', false)
...
export const useSettingsAudioDevice = defineStore('settings-audio-devices', () => {
  ...
  watch([selectedAudioInputEnabledPersist, audioInputs, selectedAudioInputPersist], ([enabled, inputs, selected]: [boolean, MediaDeviceInfo[], string]) => {
    if (!enabled)
      return
