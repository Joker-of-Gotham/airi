...
"${inputText}"`
      ...
      const fenceStart = rawContent.indexOf('```')
      if (fenceStart >= 0) {
        const fenceBodyStart = rawContent.startsWith('```json', fenceStart)
          ? fenceStart + '```json'.length
          : fenceStart + '```'.length
        const fenceBody = rawContent.slice(fenceBodyStart).trimStart()
        const fenceEnd = fenceBody.indexOf('```')
        if (fenceEnd >= 0)
          jsonContent = fenceBody.slice(0, fenceEnd).trim()
